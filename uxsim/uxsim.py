...
class Node:
    ...
    def override_signal(s, signal, groups=None, signal_offset=0, reset=False, signal_offset_old=None):
        """
        ...
        for i, group in enumerate(groups):
            for link in group:
                l = s.W.get_link(link)
                if l not in s.inlinks.values():
                    raise ValueError(f"Link {l.name} is not an incoming link of node {s.name}.")
